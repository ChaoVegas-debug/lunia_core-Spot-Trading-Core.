...
from ..metrics import (equity_total_usd, ops_capital_cap_pct,
                       spot_alloc_strategy_usd, tradable_equity_usd)
    ...
    def enforce_limits(
        ...
        *,
        ...
        exposure_limit = float(
            symbol_limits.get("max_symbol_exposure_pct", self.max_symbol_exposure_pct)
        )
        ...
        if (
            exposure_pct + (notional / max(symbol_limits.get("equity", notional), 1.0))
            > exposure_limit
        ):
