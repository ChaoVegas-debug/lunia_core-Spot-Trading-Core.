...
from ..metrics import (orders_rejected_total, orders_total, pnl_total,
                       spot_daily_pnl_usd, spot_pnl_total_usd,
                       spot_positions_open, spot_risk_reject_total,
                       spot_success_rate_pct, spot_trades_total)
from ..portfolio.portfolio import Portfolio
from ..risk.manager import RiskManager
from ..state import get_state
from ..utils.quote_detector import build_symbol, get_current_quote
...
class Agent:
        ...
        *,
        ...
        if runtime.get("global_stop") or not runtime.get("trading_on", True):
            ...
            orders_rejected_total.labels(
                symbol=symbol, side=side.upper(), reason=reason
            ).inc()
            spot_risk_reject_total.labels(reason=reason).inc()
            return {"ok": False, "reason": reason}
        side_upper = side.upper()
        logger.info(
            "Agent received spot order symbol=%s side=%s qty=%.8f",
            symbol,
            side_upper,
            qty,
        )
        ...
        quote_currency = get_current_quote()
        portfolio_equity = self.portfolio.get_equity_usd(
            {quote_currency: equity_runtime}
        )
        ...
        risk_context = abuse_context.copy() if abuse_context else {}
        risk_context.setdefault("source", "agent")
        ok, reason = self.risk.validate_order(
            equity_usd=equity,
            order_value_usd=notional,
            leverage=1.0,
            idempotency_key=idempotency_key,
            abuse_context=risk_context,
            symbol=symbol,
            side=side_upper,
        )
        if not ok:
            ...
            orders_rejected_total.labels(
                symbol=symbol, side=side_upper, reason=reason
            ).inc()
            spot_risk_reject_total.labels(reason=reason).inc()
        ...
        ok, reason = self.risk.validate_spot_order(
            ...
            limits={
                ...
                "max_symbol_exposure_pct": runtime["spot"].get(
        ...
        spot_trades_total.labels(
            strategy=strategy or "unknown", symbol=symbol, side=side_upper
        ).inc()
        record.update(
            {
    ...
    def execute_signals(
        ...
        for signal in decision.get("signals", []):
            default_symbol = build_symbol("BTC")
            symbol = str(signal.get("symbol") or default_symbol)
