...
from ..metrics import (arbitrage_avg_spread_pct, arbitrage_opportunities_total,
...
class ArbitrageEngine:
    ...
    def _get_price(self, exchange: str, symbol: str) -> Optional[float]:
        ...
        try:
            ...
            logger.warning(
                "Failed to fetch price from %s for %s: %s", exchange, symbol, exc
            )
            return None
        ...
        for pair in self.config.pairs:
            ...
            if spread_pct >= self.config.spread_threshold_pct:
                ...
                arbitrage_opportunities_total.labels(
                    symbol=pair.symbol, buy=buy_ex, sell=sell_ex
                ).inc()
