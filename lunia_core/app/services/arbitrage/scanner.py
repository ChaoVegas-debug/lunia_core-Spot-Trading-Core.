...
from app.core.metrics import (arb_filtered_out_total,
                              arb_net_profit_usd_bucket,
                              arb_net_roi_pct_bucket,
                              arb_proposals_after_filter_total,
                              arb_proposals_total, arb_qty_suggested_usd,
                              arb_scans_total)
...
class ArbitrageScanner:
    ...
    def _evaluate(
        self, symbol: str, buy: str, sell: str
        ...
        try:
            ...
            logger.warning(
                "price lookup failed symbol=%s buy=%s sell=%s err=%s",
                symbol,
                buy,
                sell,
                exc,
        ...
        spread_bps_buy = float(
            buy_limits.get("spread_bps", symbol_limits.get("spread_bps", 5.0))
        )
        spread_bps_sell = float(
            sell_limits.get("spread_bps", symbol_limits.get("spread_bps", 5.0))
        )
        ...
        transfer_type = (
            "internal"
            if buy_limits.get("internal_transfer")
            and sell_limits.get("internal_transfer")
            else "chain"
        )
        transfer_fee_usd = withdraw_fee_usd + deposit_fee_usd
        transfer_eta = float(
            self._limits.get("transfer_eta_sec", {}).get(transfer_type, 60.0)
        )
        if transfer_type == "internal":
            transfer_fee_usd = float(self._limits.get("transfer_internal_fee_usd", 0.0))
        depth_buy = float(
            buy_limits.get("depth_usd", symbol_limits.get("depth_usd", 100000.0))
        )
        depth_sell = float(
            sell_limits.get("depth_usd", symbol_limits.get("depth_usd", 100000.0))
        )
        qty_usd = self._suggest_qty_usd(
            symbol, buy_limits, sell_limits, symbol_limits, depth_buy, depth_sell
        )
        fees_total_pct = (
            taker_buy
            + taker_sell
            + (transfer_fee_usd / qty_usd * 100 if qty_usd else 0.0)
        )
        ...
        latency_ms = float(
            max(
                buy_limits.get("latency_ms", 200.0),
                sell_limits.get("latency_ms", 200.0),
            )
        )
        meta = {
            "fees": {
    ...
    def _suggest_qty_usd(
        ...
        liquidity_cap = max(
            min(available_buy, available_sell, depth_buy, depth_sell) * 0.25, min_qty
        )
