...
def build_buttons(
    top: int, auto_mode: bool, exec_mode: str
) -> List[List[Dict[str, str]]]:
    rows: List[List[Dict[str, str]]] = []
    select_row = [
        {"text": f"Выбрать {i+1}", "callback_data": f"arb_select:{i}"}
        for i in range(top)
    ]
    ...
    rows.append(
        [
            ...
            {
                "text": "Dry-Run" if exec_mode != "dry" else "Real?",
                "callback_data": "arb_toggle_exec",
            },
        ]
    )
    rows.append(
