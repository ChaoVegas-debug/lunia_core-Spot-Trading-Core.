...
from app.core.metrics import (arb_execs_total, arb_execution_latency_ms,
                              arb_fail_total, arb_net_profit_total_usd,
                              arb_success_total)
    ...
    def execute(
        ...
        *,
        ...
        ok, reason = self.risk.validate_order(
            ...
            side="BUY",
            abuse_context={"source": "arb_executor", "mode": mode},
        )
        if not ok:
        ...
        steps.append(
            {"stage": "reserve", "status": "ok", "qty_usd": opportunity.qty_usd}
        )
