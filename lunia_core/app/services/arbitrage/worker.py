...
from app.core.metrics import (arb_auto_execs_total, arb_daily_pnl_usd,
                              arb_net_profit_total_usd, arb_success_rate,
...
from app.db.reporting import arbitrage_daily_pnl, arbitrage_daily_summary
...
class RuntimeSnapshot:
    ...
    def register_execution(
        self, result: ArbitrageExecutionResult, auto_trigger: bool
    ) -> None:
...
def _init_components() -> None:
    ...
    if _SCANNER is None:
        ...
        _SCANNER = ArbitrageScanner(
            exchanges=exchanges, symbols=symbols, qty_usd=qty_usd
        )
...
def execute_by_id(
    ...
    *,
    ...
    assert _SCANNER is not None
    current = next(
        (opp for opp in _SCANNER.last_opportunities if opp.proposal_id == proposal_id),
        None,
    )
    if current is None:
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Arbitrage worker")
    parser.add_argument(
