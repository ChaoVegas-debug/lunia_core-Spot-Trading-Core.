...
def test_ops_capital_endpoints(monkeypatch):
    monkeypatch.setenv("OPS_API_TOKEN", "ops-secret")
    from app.api.middleware import auth as auth_middleware

    auth_middleware._JWT_MANAGER = auth_middleware.JWTManager(
        legacy_token=os.getenv("OPS_API_TOKEN")
    )
    ...
    update_resp = client.post(
        ...
        headers={"X-Admin-Token": os.getenv("OPS_API_TOKEN")},
    )
